...
export class Chart extends EventTarget {
    ...
    setElement(el) {
        ...
        if (!this._merge) {
            ...
            for (const tooltip of this._tooltips.values()) {
                tooltips.append(tooltip.positioner);
        ...
        this._tooltipGraphicsEl = this._rootSvgEl.querySelector(':scope > .sc-tooltip-graphics');
    ...
    addTooltip(id, options={}) {
        ...
        options.padding ??= [this.padding[0], this.padding[2]];
        ...
        this.el.addEventListener('pointermove', ev => {
            ...
            af = requestAnimationFrame(() => {
                if (!pointerAborter.signal.aborted) {
                    this._setTooltipPosition(tooltip, {x, internal: true});
                }
            });
        }, {signal});
        const x = (ev.x - state.chartOffsets[0]) * this.devicePixelRatio;
    ...
    updateVisibleTooltips(options) {
        const root = this.isParentChart() ? this : this.parentChart;
        if (root) {
            for (const tooltip of root._tooltips.values()) {
                if (tooltip.state.visible && root._isTooltipAvailable(tooltip)) {
                    root._updateTooltip(tooltip, options);
                }
            }
        }
    }
    ...
    _updateTooltip(tooltip, options={}) {
        ...
        for (let i = 0; i < state.charts.length; i++) {
            ...
            contents.push({
                ...
                entry,
                coordinates,
                element: chart.onTooltip({entry, tooltip})
            });
    ...
    _drawTooltip(tooltip, contents) {
        ...
        const state = tooltip.state;
        const top = tooltip.options.padding[0] * this.devicePixelRatio;
        const bottom = this._boxHeight - tooltip.options.padding[1] * this.devicePixelRatio;
        const centerX = contents.reduce((agg, o) => agg + o.coordinates[0], 0) / contents.length;
        const centerY = top + (bottom - top) / 2;
        if (!tooltip.graphics.length) {
            const line = createSVG({
                name: 'path',
                class: ['sc-line', 'sc-vertical']
            });
            tooltip.graphics.push(line);
            this._tooltipGraphicsEl.append(line);
        }
        const vertLine = tooltip.graphics[0];
        const existingHLines = tooltip.graphics.filter(x => x._ident === 'horiz-line');
        const existingDots = tooltip.graphics.filter(x => x._ident === 'highlight-dot');
        ...
        for (let i = 0; i < contents.length; i++) {
            ...
            let dot = existingDots.pop();
            if (!dot) {
                dot = createSVG({name: 'circle', class: 'sc-highlight-dot'});
                dot._ident = 'highlight-dot';
                tooltip.graphics.push(dot);
                vertLine.after(dot);
            }
            ...
            if (Math.abs(x - centerX) > 1) {
                let horizLine = existingHLines.pop();
                if (!horizLine) {
                    horizLine = createSVG({name: 'path', class: ['sc-line', 'sc-horizontal']});
                    horizLine._ident = 'horiz-line';
                    tooltip.graphics.push(horizLine);
                    vertLine.before(horizLine);
                }
                horizLine.setAttribute(
                    'd', `M ${x}, ${y} L ${centerX}, ${Math.min(bottom, Math.max(y, top))}`);
            }
        }
        for (const x of existingHLines) {
            x.remove();
            tooltip.graphics.splice(tooltip.graphics.indexOf(x), 1);
        }
        for (const x of existingDots) {
            x.remove();
            tooltip.graphics.splice(tooltip.graphics.indexOf(x), 1);
        }
        let vAlign, hAlign;
